...
const CLUSTER_NAMES = {
  TOP_FREE: ['top free', 'top free games', 'top free apps'],
  TOP_PAID: ['top paid', 'top paid games', 'top paid apps'],
  GROSSING: ['top grossing', 'top grossing games', 'top grossing apps'],
  TRENDING: ['trending'],
  NEW_FREE: ['top new free android apps', 'top new free games'],
  NEW_PAID: ['top new paid android apps', 'top new paid games']
};
...
function findCluster (categoryObject, collection) {
  ...
  const selectedCollection = allCollections.filter((data) =>
    ...
    CLUSTER_NAMES[collection].includes(R.path(CLUSTER_NAME_MAPPING, data).toLowerCase())
  );
...
function parseCategoryApps (categoryObject, opts) {
  ...
  if (!clusterUrl) {
    console.log('here why its empty!');
    return [];
  }
