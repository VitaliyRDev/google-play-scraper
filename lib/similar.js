...
const UPDATED_INITIAL_MAPPINGS = {
  cluster: ['ds:6', 1, 1, 0, -1, 1, 2, -1, 2],
  apps: ['ds:3', 0, 1, 0, -1, 0],
  updated: true
};
...
function parseSimilarApps (similarObject, opts) {
  ...
  return request(options, opts.throttle)
    .then(scriptData.parse)
    .then((htmlParsed) => processPages(htmlParsed, opts, [], UPDATED_INITIAL_MAPPINGS));
}

function getParsedCluster (similarObject) {
  const clusterUrl = R.path(UPDATED_INITIAL_MAPPINGS.cluster, similarObject);
