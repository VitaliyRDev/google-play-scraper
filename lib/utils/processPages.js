...
async function processPages (html, opts, savedApps, mappings) {
  ...
  let mainApp;
  if (mappings.updated) {
    mainApp = scriptData.extractor({
      appId: ['ds:4', 0, 1, 0, 23, 16, 11, 0, 0],
      title: ['ds:4', 0, 1, 0, 23, 16, 2, 0, 0],
      url: ['ds:4', 0, 1, 0, 23, 16, 2, 41, 0, 2],
      icon: ['ds:4', 0, 1, 0, 23, 16, 2, 95, 0, 3, 2],
      scoreText: ['ds:4', 0, 1, 0, 23, 16, 2, 51, 0, 0],
      score: ['ds:4', 0, 1, 0, 23, 16, 2, 51, 0, 1],
      developer: ['ds:4', 0, 1, 0, 23, 16, 2, 68, 0],
      summary: ['ds:4', 0, 1, 0, 23, 16, 2, 73, 0, 1]
    })(html);
  }

  if (mainApp && mainApp.appId) mappings.apps = ['ds:4', 0, 1, 2, 22, 0];
  const processedApps = appList.extract(mappings.apps, html, mappings.updated ? 'UPDATED' : 'REGULAR');

  if (mainApp && mainApp.appId) processedApps.unshift(mainApp);
  ...
  let token;
  if (mappings.token) token = R.path(mappings.token, html);
